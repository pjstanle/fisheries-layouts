...
def evaluate_plant(x):
    ...
    # middle_x = np.mean(turbine_x)
    # middle_y = np.mean(turbine_y)
    # middle_x = (turbine_x[48]+turbine_x[35])/2.0
    # middle_y = (turbine_y[48]+turbine_y[35])/2.0
    ...
    plot_turbines(turbine_x,turbine_y,rotor_diameter/2.0,color="C0",nums=True)
    plt.axis("equal")
    plt.pause(0.0001)
...
if __name__=="__main__":
    ...
    center_x = (12262.352214377384+2770.858882647537)/2-2770.858882647537
    center_y = (13971.20572433466+68.42766176234385)/2-68.42766176234385
    ...
    x["shear"] = start_shear
    x["x_spacing"] = start_x_spacing-600
    x["y_spacing"] = start_y_spacing-600
